...
namespace quik {
    ...
    public:
        ...
        virtual ~read_file() = default;

        /**
         * Add another read to the set.
         * @param sequence ASCII Sequence.
         */
        read add(const std::string& sequence) {
            return add(sequence, "read" + std::to_string(size()));
        }
        ...
        read add(const std::string& sequence, const std::string& fastq_id) {
            ...
            assert(read_data.size() == read_data_start_index[read_id+1]);
        ...
        const std::string& get_fastq_id(unsigned read_id) const {
        ...
        read operator[](size_t read_id) const {
            assert(read_id < size());
            auto start_ptr = read_data.data() + read_data_start_index[read_id];
            return {start_ptr, read_sequence_lengths[read_id]};
        }
