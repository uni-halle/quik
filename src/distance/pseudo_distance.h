...
__host__ __device__ int pseudo_distance(const barcode &b, const read &r) {
    ...
    for (int i = 0; i + k <= BARCODE_LENGTH; i++) {
        ...
        for (; j + k <= READ_LENGTH && j <= i + window_size; j++) {
...
inline int pseudo_distance(const barcode &b, const read &r, unsigned k) {
    switch (k) {
        case 1:
