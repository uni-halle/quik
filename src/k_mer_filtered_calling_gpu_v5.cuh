...
>
__global__ void calling_kernel_kmer_filter_gpu_v5(
        ...
        unsigned *index_barcode_ids,           // for each read: array of size MAX_INDEX_SIZE
        int16_t *index_sl_distance,            // for each read: array of size MAX_INDEX_SIZE
        ...
        const size_t read_count)          // number of reads
{
    ...
    for (unsigned read_id = thread_id; read_id < read_count; read_id += thread_count) {
        ...
        assert(MAX_INDEX_SIZE < barcode_count);
        ...
        for (unsigned p = 0; p < barcode_candidate_count; p++) {
            ...
            // select the p-th candidate in each thread's candidate list and its associated pseudo distance
            ...
            for (; j > 0 && index_sl_distance[(j - 1) * read_count + read_id] > q; j--) {
                if (j < MAX_INDEX_SIZE) {
                    index_sl_distance[j * read_count + read_id] = index_sl_distance[(j - 1) * read_count + read_id];
                    index_barcode_ids[j * read_count + read_id] = index_barcode_ids[(j - 1) * read_count + read_id];
                }
            }

            if (j < MAX_INDEX_SIZE) {
                index_sl_distance[j * read_count + read_id] = q;
                index_barcode_ids[j * read_count + read_id] = barcode_id;
            }
        ...
        /*if (read_id == 1) {
            ...
            for (unsigned l = 0; l < my_local_index_size; l++) {
                printf("pos=%i, barcode=%u, distance=%i\n", l, index_barcode_ids[l*read_count+read_id], index_distance[l*read_count+read_id]);
            }
        }*/
        ...
        for (unsigned rank = 0; rank < MAX_INDEX_SIZE && rank < barcode_count; rank++) {
            barcode b = barcodes[index_barcode_ids[rank * read_count + read_id]];
            index_sl_distance[rank * read_count + read_id] = exact_distance(b, r, UINT8_MAX);
        }
        ...
        for (unsigned i = 1; i < my_local_index_size; i++) {
            ...
            // insert the index element i into the sorted index[0...i-1]
            unsigned barcode_i = index_barcode_ids[i * read_count + read_id];
            int16_t sl_dist_i = index_sl_distance[i * read_count + read_id];

            unsigned j = i;
            for (; j > 0; j--) {

                int16_t sl_dist_j_1 = index_sl_distance[(j - 1) * read_count + read_id];
                //unsigned barcode_j_1 = index_barcode_ids[(j - 1) * read_count + read_id];

                if (sl_dist_j_1 > sl_dist_i) {
                    index_barcode_ids[j * read_count + read_id] = index_barcode_ids[(j - 1) * read_count + read_id];
                    index_sl_distance[j * read_count + read_id] = index_sl_distance[(j - 1) * read_count + read_id];
                } else
                    break;
            }
            index_barcode_ids[j * read_count + read_id] = barcode_i;
            index_sl_distance[j * read_count + read_id] = sl_dist_i;
        }

        /*if (read_id == 1) {
            printf("index of read %i: after step 4\n", read_id);
            for (unsigned l = 0; l < my_local_index_size; l++) {
                printf("pos=%i, barcode=%i, distance=%i\n", l, index_barcode_ids[l * read_count + read_id], index_sl_distance[l * read_count + read_id]);
            }
        }*/
...
struct k_mer_filtered_calling_gpu_v5 : public barcode_assignment {

    k_mer_filtered_calling_gpu_v5(const std::vector<barcode> &barcodes,
...
private:
    ...
    barcode_assignment run(const std::vector<barcode> &barcodes,
        ...
        unsigned *out_index_barcodes_dev;
        int16_t *out_index_sl_distances_dev;

        cudaMalloc(&out_index_barcodes_dev, reads.size() * MAX_INDEX_SIZE * sizeof(unsigned)); // todo: cudaMallocPitch
        cudaMalloc(&out_index_sl_distances_dev, reads.size() * MAX_INDEX_SIZE * sizeof(int16_t)); // todo: cudaMallocPitch
...
#pragma omp parallel for
        ...
        /************************************************************************************************
         * Calculate the maximum number of threads such that each thread can have two arrays of size
         * barcode_count, each.
         ************************************************************************************************/

        static constexpr unsigned threads_per_block = 32;
        unsigned block_count = (unsigned) (0.9 * (double) free_mem /
                                           (double) (threads_per_block * barcodes.size() * (sizeof(unsigned) + sizeof(int16_t))));
        ...
        calling_kernel_kmer_filter_gpu_v5
        <k, threads_per_block, exact_distance>
        <<<block_count, threads_per_block>>>(
                barcodes_dev, reads_dev, map_entries_dev, start_index_dev, out_index_barcodes_dev,
                        out_index_sl_distances_dev, out_pseudo_distances_dev, barcode_candidates_dev, barcodes.size(), reads.size()
        );
        ...
        int16_t *out_index_sl_distances_host = new int16_t[reads.size() * MAX_INDEX_SIZE];

        cudaMemcpy(&ass[0], out_index_barcodes_dev, reads.size()*sizeof(unsigned), cudaMemcpyDeviceToHost );
        cudaMemcpy(out_index_sl_distances_host, out_index_sl_distances_dev, reads.size() * sizeof(int16_t), cudaMemcpyDeviceToHost);

#pragma omp parallel for
        for(unsigned read_id = 0; read_id < reads.size(); read_id++){
            if(out_index_sl_distances_host[read_id] > rejection_threshold) {
                ass[read_id] = UINT_MAX;
                /*if(read_id == 1)
                    std::cout << "reject read " << read_id << " with distance " << out_index_sl_distances_host[read_id] << std::endl;*/
            }
        }
