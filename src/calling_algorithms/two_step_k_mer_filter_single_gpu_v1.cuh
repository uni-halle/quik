...
namespace quik {
    ...
    class two_step_k_mer_filter_single_gpu_v1 : public barcode_calling_algorithm {

        const distance_function distance;

    public:
        two_step_k_mer_filter_single_gpu_v1(
        ...
        static std::string name() {
            return std::to_string(k_large) + "_" +
                std::to_string(k_small) + "_mer_filter_single_gpu_v1";
        }

        /**
         * Run the algorithm
         **/
        two_step_k_mer_filter_single_gpu_v1& run() override {

            /************************************************************************************************
             * Start by calculating a barcode assignment using k_large-mers
             ************************************************************************************************/

            k_mer_filter_single_gpu_default<k_large, distance_function>
                ass_k_large(barcodes, reads, rejection_threshold, distance);
            ass_k_large.run();
            ...
            k_mer_filter_single_gpu_default<k_small, distance_function> ass_k_small(
            ...
            for (unsigned unassigned_read_id = 0; unassigned_read_id < unassigned_reads.size(); unassigned_read_id++) {
                ...
                //std::cout << "assign jet uassigned read " << read_id << " to barcode " << barcode_id_1 << std::endl;
