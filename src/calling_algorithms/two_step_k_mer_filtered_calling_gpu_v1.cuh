    ...
    public:
        two_step_k_mer_filtered_calling_gpu_v1(
            ...
            : barcode_calling_algorithm(std::to_string(k_large) + "_" +
                                        std::to_string(k_small) + "_mer_filtered_calling_gpu_v1", dist),
        ...
        extended_barcode_assignment run(
            ...
            auto ass_k_large = k_mer_filtered_calling_gpu_v5<k_large>(dist).run(barcodes, reads);
            ...
            for (unsigned read_id = 0; read_id < reads.size(); read_id++) {
                ...
                if (ass_k_large.get_distance_to_1st_barcode(read_id) > rejection_threshold) {
                    ...
                    unassigned_reads.add(reads[read_id].to_string(), reads.get_name_of(read_id));
                }
            }
            ...
            auto ass_k_small = k_mer_filtered_calling_gpu_v5<k_small>(dist).run(barcodes, unassigned_reads);
