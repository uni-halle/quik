...
namespace barcode_calling {
    ...
    class k_mer_filtered_calling_host_v1 : public barcode_calling_algorithm {

    protected:
        const distance_measure& dist;

    public:
        explicit k_mer_filtered_calling_host_v1(const distance_measure& dist)
            : barcode_calling_algorithm(std::to_string(k) + "_mer_filtered_calling_host_v1"),
              dist(dist) {}
        ...
        extended_barcode_assignment run(
            ...
            extended_barcode_assignment ass(reads.size());
            ...
            static_assert(2 * k < 32, "Invalid Parameter k");
            ...
            for (unsigned read_id = 0; read_id < reads.size(); read_id++) {
                ...
                unsigned PSEUDO_DISTANCE_DEFAULT = BARCODE_LENGTH * BARCODE_LENGTH * BARCODE_LENGTH;
                ...
                /****************************************************************************************************
                 * Step 3: Replace the pseudo distances by more exact distances for each candidate barcode.
                 ***************************************************************************************************/

                for (unsigned i = 0; i < index_size; i++) {
                    index[i].distance = dist(barcodes[index[i].barcode_id], r);
                }
