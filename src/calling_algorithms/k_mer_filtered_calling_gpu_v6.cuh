...
#ifndef INC_2OPT_KMER_FILTERED_CALLING_GPU_V6_CUH
#define INC_2OPT_KMER_FILTERED_CALLING_GPU_V6_CUH
...
namespace barcode_calling {
    ...
    namespace k_mer_filtered_calling_gpu_v6_detail {
        ...
        inline size_t compute_shared_bytes(unsigned barcodes_per_chunk) {
            ...
            offset += (barcodes_per_chunk + 1) * sizeof(int32_t);
         ...
         * @return
         */
        __device__ inline unsigned arg_min_intra_block(unsigned* index_set, const int32_t* keys, unsigned size) {
            unsigned active_elements = size;
            while (active_elements > 1) {
                unsigned half = (active_elements + 1) / 2; // half = ceil(active_elements/2)
                for (unsigned i = threadIdx.x; half + i < active_elements; i += 32) {
                    unsigned j = half + i;
        ...
        __global__ void kernel(
            ...
            assert(barcodes_per_chunk % 32 == 0);
             ...
             *
             *        pseudo_distance = int32_t[barcodes_per_chunk+1]
             *
             *    The final entry pseudo_distance[barcodes_per_chunk] is INT_MAX.
             *
            ...
            for (unsigned i = threadIdx.x; i <= CANDIDATE_COUNT; i += 32) {
                candidate_barcodes[i] = barcodes_per_chunk;
            ...
            for (unsigned chunk_id = 0; chunk_id < chunk_count; ++chunk_id) {
                ...
                for (unsigned barcode_id = threadIdx.x; barcode_id < barcodes_per_chunk; barcode_id += 32)
                    pseudo_distance[barcode_id] = 0;
                if (threadIdx.x == 0)
                    pseudo_distance[barcodes_per_chunk] = INT32_MAX;
                __threadfence_block();
                ...
                for (int i = 0; i + k <= r.length(); i++) {
                    ...
                    for (; j + k <= BARCODE_LENGTH && j <= i + PSEUDO_DISTANCE_WINDOW_SIZE; j++) {
                        ...
                        for (size_t l = start + threadIdx.x; l < stop; l += 32) {
                 ...
                 *
                 * To this end, we partition the barcode of this chunk between the threads such that
                 * each thread is associated to a subset of barcodes_per_chunk / 32 barcodes.
                 * Each thread stores these barcodes in sorted order within its local memory.
                 *
                 * We use insertion sort as the number of barcodes per thread is very small (about 64).
                 *
                 * For each thread, we keep track of the barcode with the current minimum pseudo distance
                 * using an index my_min_index.
                 ****************************************************************************************/

                constexpr unsigned len = barcodes_per_chunk / 32;
                unsigned my_barcode_ids[len + 1];
                for (unsigned i = 0; i < len; i++)
                    my_barcode_ids[i] = i * 32 + threadIdx.x;
                my_barcode_ids[len] = barcodes_per_chunk;

                // sort the barcode ids by their pseudo distance (via insertion sort)
                for (unsigned i = 1; i < len; i++) {
                    unsigned barcode_id_i = my_barcode_ids[i];
                    int32_t pseudo_dist_i = pseudo_distance[barcode_id_i];
                    unsigned j = i;
                    for (; j > 0 && pseudo_distance[my_barcode_ids[j - 1]] > pseudo_dist_i; --j)
                        my_barcode_ids[j] = my_barcode_ids[j - 1];
                    my_barcode_ids[j] = barcode_id_i;
                }

                unsigned my_min_index = 0;
                ...
                for (unsigned rank = 0; rank < CANDIDATE_COUNT; rank++) {
                     ...
                     *
                     * For this purpose, each thread puts the id of its current minimum barcode into
                     * the shared memory. Then, we use an in-block-reduction to determine the minimum barcode
                     * within the whole block.
                     *
                     * The thread whose barcode_id won the reduction then increments its min_index.
                     ****************************************************************************************/

                    assert(my_min_index <= len);
                    barcode_ids[threadIdx.x] = my_barcode_ids[my_min_index];
                    unsigned best_barcode_in_chunk = arg_min_intra_block(barcode_ids, pseudo_distance, 32);
                    int32_t best_pseudo_dist_in_chunk = pseudo_distance[best_barcode_in_chunk];

                    /*****************************************************************************************
                     * The winner thread updates its index. No syncthreads required as we only have one warp
                     ****************************************************************************************/

                    if (best_barcode_in_chunk == my_barcode_ids[my_min_index])
                        my_min_index++;
            ...
            for (unsigned i = threadIdx.x; i < CANDIDATE_COUNT; i += 32) {
                ...
                //printf("candidate_barcode[%i]=%u, candidate_distance[%i]=%i\n", i, candidate_barcodes[i], i, candidate_distance[i]);
            }
    ...
    template <unsigned k,
              unsigned barcodes_per_chunk = 512>
    ...
    public:
        ...
        extended_barcode_assignment run(
            ...
            size_t shared_size = k_mer_filtered_calling_gpu_v6_detail::
            ...
            ************************************************************************************************/
            ...
            k_mer_filtered_calling_gpu_v6_detail::kernel
...
#endif //INC_2OPT_KMER_FILTERED_CALLING_GPU_V6_CUH
