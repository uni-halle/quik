...
namespace barcode_calling {
    ...
    public:
        ...
        static constexpr char _nucleotides[] = {'A', 'C', 'G', 'T'};

        static void string_to_bytes(const std::string& str, uint8_t* bases) {

            unsigned array_length = (str.length() + 4 - 1) / 4;
         ...
         * @return
         */
        __host__ __device__ static base extract_i_th_nucleotide(
            const uint8_t* sequence_data, unsigned index) {
        ...
        /**
         * Return the number of bytes required to store this read.
         * @return
         */
        __host__ __device__
        ...
        __host__ __device__
        void bytes_to_string(char* dst) const {
            for (unsigned i = 0; i < length(); i++)
                dst[i] = operator[](i).to_char();
            dst[length()] = '\0';
        }
    };
}
